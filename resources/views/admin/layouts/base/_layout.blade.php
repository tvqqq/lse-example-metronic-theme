                ...
                <div class="content {{ Metronic::printClasses('content', false) }} d-flex flex-column flex-column-fluid" id="kt_content">
                    ...
                    @if(config('layout.subheader.display'))
                        @if(array_key_exists(config('layout.subheader.layout'), config('layout.subheader.layouts')))
                            @include('admin.layouts.partials.subheader._'.config('layout.subheader.layout'))
                        @else
                            @include('admin.layouts.partials.subheader._'.array_key_first(config('layout.subheader.layouts')))
                        @endif
                    @endif

                    @include('admin.layouts.base._content')
                </div>
...
@if (config('layout.self.layout') != 'blank')
    ...
    @if (config('layout.extras.search.layout') == 'offcanvas')
        @include('admin.layouts.partials.extras.offcanvas._quick-search')
    @endif

    @if (config('layout.extras.notifications.layout') == 'offcanvas')
        @include('admin.layouts.partials.extras.offcanvas._quick-notifications')
    @endif

    @if (config('layout.extras.quick-actions.layout') == 'offcanvas')
        @include('admin.layouts.partials.extras.offcanvas._quick-actions')
    @endif

    @if (config('layout.extras.user.layout') == 'offcanvas')
        @include('admin.layouts.partials.extras.offcanvas._quick-user')
    @endif

    @if (config('layout.extras.quick-panel.display'))
        @include('admin.layouts.partials.extras.offcanvas._quick-panel')
    @endif

    @if (config('layout.extras.toolbar.display'))
        @include('admin.layouts.partials.extras._toolbar')
    @endif

    @if (config('layout.extras.chat.display'))
        @include('admin.layouts.partials.extras._chat')
    @endif

    @include('admin.layouts.partials.extras._scrolltop')
